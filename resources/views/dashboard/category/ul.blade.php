<div class="mr-4" x-data={show:false}>

    <li class="p-1 text-gray-500 cursor-pointer" @click="show=!show">

        @if ($children->children->count())
            <ion-icon class="text-sm align-middle " name="chevron-back-outline"></ion-icon>
            </ion-icon>
            <span x-show="show" class="list-line"> </span>
        @else
            <ion-icon class="text-sm align-middle " name="remove-outline"></ion-icon>
        @endif
        <span class="text-sm">
            {{ $children->title }}
        </span>

        <span class="px-2" x-data="{var{{ $children->id }}:false , edit{{ $children->id }}:false}">
            @if (!$children->children->count())
                <a href="/specs/{{ $children->id }}" class=" text-cool-gray-400 hover:text-teal-700">
                    <ion-icon name="git-merge-outline"></ion-icon>
                </a>
            @endif

            <button @click="edit{{ $children->id }} = true" class=" text-cool-gray-400 hover:text-indigo-700">
                <ion-icon name="create"></ion-icon>
            </button>
            <button @click=" var{{ $children->id }}=true" class="text-yellow-400 hover:text-yellow-700">
                <ion-icon name="trash"></ion-icon>
            </button>

            <div x-show="var{{ $children->id }}" class="fixed inset-0 z-10 overflow-y-auto">
                <div class="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                    ...
                    <div class="inline-block overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl modal-anim sm:my-8 sm:align-middle sm:max-w-lg sm:w-full"
                        ...
                        <div class="px-4 py-3 bg-gray-50 sm:px-6 sm:flex sm:flex-row-reverse">
                            <form action="/category/{{ $children->id }}" method="POST">
                                @method('DELETE')
                                @csrf
                                <button type="submit"
                            ...
                            <button @click="var{{ $children->id }} = false" type="button"
            ...
            <div x-show="edit{{ $children->id }}" class="fixed inset-0 z-10 overflow-y-auto">
                <div class="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                    ...
                    <div class="inline-block overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl modal-anim sm:my-8 sm:align-middle sm:max-w-3xl sm:w-full"
                        role="dialog" aria-modal="true" aria-labelledby="modal-headline">
                        <form method="POST" enctype="multipart/form-data" action="/category/{{ $children->id }}">
                            <div class="px-4 pt-5 pb-4 bg-white sm:p-6 sm:pb-4">
                                <div
                                    ...
                                    <div class="px-4 py-5 bg-white sm:p-6">
                                        ...
                                        <div class="grid grid-cols-6 gap-6">
                                            <div class="col-span-6 mb-3 sm:col-span-3">
                                                ...
                                                <input value="{{ $children->title }}" placeholder="نام فارسی "
                                                    name="title" type="text" id="first_name"
                                            ...
                                            <div class="col-span-6 mb-3 sm:col-span-3">
                                                <label for="first_name"
                                                    class="block text-sm font-medium text-right text-gray-700">نام
                                                    انگلیسی </label>
                                                <input placeholder="نام انگلیسی " value="{{ $children->text }}"
                                                    name="text" type="text" id="first_name"
                                                    class="block w-full px-3 py-2 mt-1 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm ">
                                            </div>
                                            <div class="col-span-6 sm:col-span-2">
                                                ...
                                                <select id="view" name="category_id"
                                                    ...
                                                    @foreach ($all as $cat)
                                                        ...
                                                        @if ($cat->id === $children->category_id)
                            ...
                            <div class="px-4 py-3 bg-gray-50 sm:px-6 sm:flex sm:flex-row-reverse">
                                ...
                                <button type="button" @click="edit{{ $children->id }} = false"
                                    class="inline-flex justify-center w-full px-4 py-2 mt-3 text-base font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">
                                    لغو
                                </button>
                            </div>
    ...
    <ul x-show="show">
        @foreach ($children->children as $key => $children)
